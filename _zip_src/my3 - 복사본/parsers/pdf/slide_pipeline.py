...
def process_slide_ir(doc, page_idx, plumber_tables):
    ...
    # ── Salvage logic ──
    has_offpage_text_stream = any(
        block.get("source") == "pymupdf_slide_offpage_text_stream"
        or (block.get("meta") or {}).get("slide_role") == "offpage_text_stream"
        or (block.get("meta") or {}).get("layout_reliability") == "offpage_bbox"
        for block in blocks
        if block.get("type") in ("text", "title")
    )
    weak_offpage_native = (
        page_mode == "hybrid_slide"
        and has_offpage_text_stream
        and native_text_chars < 120
        and image_count > 0
        and len(visual_candidates) >= 100
    )
    needs_salvage = (
        not blocks
    ...
    if needs_salvage:
        ...
        near_empty_reason = (
            ...
            else "weak_offpage_native_text" if weak_offpage_native
        ...
        if (
            ...
            or (native_text_chars > 80 and not weak_offpage_native)
            or (page_mode == "hybrid_slide" and ppt_recovered_len >= 45 and not weak_offpage_native)
    ...
    return {
        ...
        "parser_debug": {
            ...
            "has_offpage_text_stream": has_offpage_text_stream,
            "weak_offpage_native_salvage": weak_offpage_native,
...
def _extract_slide_text_blocks(raw: dict, page_idx: int, pw: float, ph: float) -> list[dict]:
    ...
    for block in raw.get("blocks", []):
        ...
        for line in block.get("lines", []):
            ...
            if not is_offpage and is_visual_noise_text(text) and len(text) < 12 and not _is_reassemblable_slide_fragment(text, bbox, ph):
                continue
...
def _is_reassemblable_slide_fragment(text: str, bbox: list[float], ph: float) -> bool:
    """Keep short visible fragments that can form financial bullet/KPI lines."""
    clean = (text or "").strip()
    if not clean:
        return False
    in_title_band = len(bbox) == 4 and bbox[1] < ph * 0.14
    if re.fullmatch(r"\dQ\d{2,4}", clean, flags=re.IGNORECASE):
        return False
    if in_title_band and re.fullmatch(r"[+-]?\d+(?:[.,]\d+)?%?\)?", clean):
        return False
    if re.search(r"[A-Za-z가-힣]", clean):
        return True
    if re.fullmatch(r"[+-]?\d+(?:[.,]\d+)?%?\)?", clean):
        return True
    return bool(re.search(r"\d", clean) and re.search(r"[+%.,)]", clean))
